// src/pages/CreateInvoice.jsx (Completo con la mejora de cantidad)
...
const CreateInvoice = () => {
    ...
    const getStockForLocation = (product, location) => {
        if (!product || !product.batches || !location) return 0;
    ...
    const handleAddItem = () => {
        ...
        if (!productToAdd || quantity <= 0) {
            toast.warn("Selecciona un producto y una cantidad válida.");
            return;
        }
        ...
        if (existingItem) {
            setInvoiceItems(
                invoiceItems.map(item => 
                    ...
                    ? { ...item, quantity: newTotalQuantity, subtotal: newTotalQuantity * item.price }
                    : item
                )
            );
    ...
    const handleSubmitInvoice = async () => {
        if (!selectedClientId || !saleLocation || invoiceItems.length === 0) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-md">
                        ...
                        <button onClick={handleAddItem} className="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600 transition-colors">Añadir a la Factura</button>
                    </div>
                </div>
                                ...
                                <AnimatePresence>
                                    {invoiceItems.map(item => (
                                        ...
                                        >
                                            ...
                                            <td className="p-2 text-center"><button onClick={() => handleRemoveItem(item.productId)} className="text-red-500 hover:text-red-700"><FiTrash2 /></button></td>
                                        </motion.tr>
                                    ))}
                                </AnimatePresence>
                            </tbody>
